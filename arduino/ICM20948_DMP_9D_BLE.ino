...
// Device name
const char* nameOfPeripheral = "IMUMonitor";
...
BLECharacteristic txChar(uuidOfTxStruct, BLERead | BLENotify | BLEBroadcast, sizeof(datareading));
...
void setup()
{
  ...
  IMUService.addCharacteristic(txChar);
  ...
  SERIAL_PORT.println("Peripheral advertising info: ");
  SERIAL_PORT.print("Name: ");
  SERIAL_PORT.println(nameOfPeripheral);
  SERIAL_PORT.print("MAC: ");
  SERIAL_PORT.println(BLE.address());
  SERIAL_PORT.print("Service UUID: ");
  SERIAL_PORT.println(IMUService.uuid());
  SERIAL_PORT.print("txCharacteristics UUID: ");
  SERIAL_PORT.println(uuidOfTxStruct);
    ...
    while (central.connected()) {
      ...
      if ((myICM.status == ICM_20948_Stat_Ok) || (myICM.status == ICM_20948_Stat_FIFOMoreDataAvail)) // Was valid data available?
      {
        ...
        if ((data.header & DMP_header_bitmap_Quat9) > 0) // We have asked for orientation data so we should receive Quat9
        {
          ...
          datareading.ax = data.Raw_Accel.Data.X; // Extract the raw accelerometer data
          datareading.ay = data.Raw_Accel.Data.Y;
          datareading.az = data.Raw_Accel.Data.Z;

          float acc_x = (float)datareading.ax; // convert the raw accelerometer data
          float acc_y = (float)datareading.ay;
          float acc_z = (float)datareading.az;

          timeMs = millis();

          datareading.timeread=timeMs;
          // Here we convert data from float to int to send over BLE
          datareading.qw=(int) (q0*10000);
          datareading.qx=(int) (q1*10000);
          datareading.qy=(int) (q2*10000);
          datareading.qz=(int) (q3*10000);

          // Send the IMU values to the central device.
          txChar.writeValue((uint8_t *)&datareading, sizeof(datareading));

          // Output the Quaternion data in the format expected by ZaneL's Node.js Quaternion animation tool          
          SERIAL_PORT.print(F("{\"timeMs\":"));
          SERIAL_PORT.print(timeMs);
...
void startBLE() {
  if (!BLE.begin())
  {
    SERIAL_PORT.println("starting BLE failed!");
    while (1);
  }
}
